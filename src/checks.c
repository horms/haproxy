...
static const struct check_status check_statuses[HCHK_STATUS_SIZE] = {
	...
	[HCHK_STATUS_PROCERR]	= { SRV_CHK_ERROR,                     "PROCERR", "External check error or failure" },
	[HCHK_STATUS_PROCTOUT]	= { SRV_CHK_RUNNING,                   "PROCOK",  "External check timeout" },
 ...
 new_chk:
	...
	if (!is_check_running(s)) {   /* no check currently running */
		...
		if ((s->proxy->options2 & PR_O2_CHK_ANY) == PR_O2_EXT_CHK) {
			...
			if (elem->exited) {
				status = elem->status; /* Save incase the process exits between the two checks */
